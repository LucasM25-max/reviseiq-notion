...
export function renderMain() {
  ...
  html += '<div class="add-block-row"><div class="add-block-ghost" id="add-block-ghost">+&nbsp; Click to add a block, or type / for commands</div></div>';
  root.innerHTML = html;
  root.scrollTop = 0;
  document.getElementById("main").scrollTop = 0;
}
...
export function renderEmptyState() {
  return (
    ...
    '<button id="empty-new-subject">+ New subject</button>' +
    "</div>"
  );
}
...
export function renderBreadcrumbs(page) {
  ...
  chain.forEach((p) => {
    html += '<span class="crumb" data-nav="' + p.id + '">' + p.icon + " " + escapeHtml(p.title || "Untitled") + '</span><span class="crumb-sep">/</span>';
  });
  html += '<span class="crumb current">' + page.icon + " " + escapeHtml(page.title || "Untitled") + "</span></div>";
  return html;
}
...
export function renderPageHeader(page) {
  return (
    ...
    '<button class="page-icon-btn" id="page-icon-btn" data-page-id="' +
    page.id +
    '">' +
    page.icon +
    "</button>" +
...
export function renderExamPanel(page) {
  ...
  html += '<button class="add-exam-btn" id="add-exam-btn">+ Add exam date</button>';
